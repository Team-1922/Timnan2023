...
public class VisionTrajectoryDrive extends CommandBase {
 ...
 // private PhotonPipelineResult result;
  ...
  public void initialize() {
    startingPose = m_poseEstimation.EstimatePose();
    timer.start();
  ...
  */
  ...
  public void execute() {
    ...
    // Perhaps use the initialize to set the particualr waypoints based on the apriltag looked at, would save a lot of processing later on down the line //

    trajectoryState = trajectory.sample(timer.get());
    SmartDashboard.putNumber("State X", trajectoryState.poseMeters.getX());
    
    chassisSpeeds = (ramseteController.calculate(m_poseEstimation.EstimatePose(), trajectoryState));
  ...
  public boolean isFinished() {
    return Math.abs(m_poseEstimation.EstimatePose().getX() - m_endingPose.getX() ) <.1
        && Math.abs(m_poseEstimation.EstimatePose().getY() - m_endingPose.getY() ) <.1;  
